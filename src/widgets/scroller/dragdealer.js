...
(function (root, factory) {
  ...
  const Dragdealer = function (wrapper, options) {
    /**
  ...
  Dragdealer.prototype = {
    ...
    getHandleElement (wrapper, handleClass) {
      let childElements,
        handleClassMatcher,
        i;
    ...
    calculateValuePrecision () {
      ...
      let xPrecision = this.options.xPrecision || Math.abs(this.bounds.availWidth),
    ...
    setValue (x, y, snap, igronEvent) {
      this.setTargetValue([ x, y || 0, ], undefined, igronEvent);
      if (snap) {
    ...
    stopDrag () {
      ...
      this.dragging = false;
      let deltaX = this.bounds.availWidth === 0 ? 0 :
      ...
      const target = this.groupClone(this.value.current);
      if (this.options.slide) {
        const ratioChange = this.change;
    ...
    setTargetValue (value, loose, igronEvent) {
      const target = loose ? this.getLooseValue(value) : this.getProperValue(value);
    ...
    setTargetValueByOffset (offset, loose) {
      const value = this.getRatiosByOffsets(offset);
      const target = loose ? this.getLooseValue(value) : this.getProperValue(value);
    ...
    getLooseValue (value) {
      const proper = this.getProperValue(value);
      return [
    ...
    getProperValue (value) {
      let proper = this.groupClone(value);
